	...
	return (
		<>
			...
			<div className="group relative h-96 w-60">
				...
				/>
				<div className="absolute -bottom-10 left-4 right-4 pointer-events-none select-none group-hover:pointer-events-auto">
					<div className="mb-2 transform-gpu text-2xl font-bold text-white delay-75 duration-200 group-hover:-translate-y-14 group-hover:delay-75 cursor-pointer">
						<Link href={`/dashboard/albums/${album.uuid}`}>{album.name}</Link>
					</div>
					...
					<Button className="transform-gpu bg-white opacity-0 transition-all duration-100 group-hover:-translate-y-14 group-hover:opacity-100 group-hover:delay-100">
						Settings
					</Button>
				</div>
			</div>
