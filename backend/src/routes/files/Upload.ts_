...
const uploadFile = async (req: RequestWithOptionalUser, res: Response) => {
	...
	if (!files.length) {
		return res.code(400).json({ message: 'No files.' });
	}
	...
	try {
		...
		return res.code(400).json({ message: error instanceof Error ? error.message : 'Bad request.' });
	}
...
const finishChunks = async (req: RequestWithOptionalUser, res: Response) => {
	...
	if (!Array.isArray(_body?.files)) return res.code(400).json({ message: 'files[] needs to be an array' });
	if (!_body.files.length) return res.code(400).json({ message: 'files[] is empty' });
	if (_body.files.some((file: any) => typeof file !== 'object' || !file.uuid))
		return res.code(400).json({ message: "One of the files is not an object or doesn't have an uuid" });
	...
	if (albumUuid) {
		if (req.user) {
			...
			if (album) {
				...
				res.code(400).json({ message: "Album doesn't exist or it doesn't belong to the user." });
			}
		} else {
			return res.code(401).json({ message: 'Only registered users can upload files to an album.' });
		}
	}
