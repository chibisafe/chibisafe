...
### Attention (2)
v4.0.1 changed the hashing algorithm for a better, faster one. So if you are currently running v4.0.0 and decide to update to v4.0.1+ it's in your best interest to rehash all the files your instance is serving. To do this go to the toshokan root folder and run `node src/api/utils/rehashDatabase.js`. Depending on how many files you have it can take a few minutes or hours, there's a progress bar that will give you an idea.
...
- Out of the box support for ShareX configuration letting you upload screenshots and screencaptures directly to your toshokan instance.
- Browser extension to be able to right click any image/video and upload it directly to your toshokan instance.
...
6. Check the [nginx](docs/nginx.md) file for a sample configuration that has every step to run toshokan securely on production.

After you finish setting up nginx, you need to start toshokan by using pm2. If you want to use something else like forever, ensure that the process spawned from `npm run start` never dies.
...
**Chibisafe** © [Pitu](https://github.com/Pitu), Released under the [MIT](https://github.com/WeebDev/toshokan/blob/master/LICENSE) License.<br>
Authored and maintained by Pitu.

> [toshokan.moe](https://toshokan.moe) · GitHub [@Pitu](https://github.com/Pitu)
